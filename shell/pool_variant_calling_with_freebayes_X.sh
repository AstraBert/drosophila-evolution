...
#freebayes="/home/mathieu/Bureau/ANALYSES/ProgDIVERS/FREEBAYES/freebayes-1.3.6-linux-amd64-static"

BAMLIST=$1
REGION=$2
ploidyfile=$3
PATH_TO_ASSEMBLY="/work/project/gandhi/DSU_assembly/dsu_isojap1_ncbi_Mito_wRI.fa"

freebayes -f $PATH_TO_ASSEMBLY -L $BAMLIST  -r $REGION -K -C 1 -F 0.01 -G 5 --limit-coverage 500 -n 4 -m 30 -q 20 -A $ploidyfile  |gzip -c > res_vcf/${BAMLIST}.${REGION}.freebayes.vcf.gz
#freebayes -f $PATH_TO_ASSEMBLY -L $BAMLIST  -r $REGION -K -E -1 -C 1 -F 0.01 -G 5 --limit-coverage 500 -n 4 -m 30 -q 20 -A $ploidyfile  |gzip -c > res_vcf/${BAMLIST}.${REGION}.freebayes2.vcf.gz
