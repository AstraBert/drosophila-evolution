...
rm(dum.DGN,tmp.nom,tmp.nomfp.DGN,tmp.ind.DGN,tmp.nomtp)
...
for(i in 1:nfake.pools){
tmp.refcount=cbind(tmp.refcount,rowSums(tmp@randomallele.pca[,fakepool.ind.idx.DGN[[i]]]))
tmp.readcov=cbind(tmp.readcov,rowSums(tmp@readcoverage[,fakepool.ind.idx.DGN[[i]]]))
}
if(nctg.pp==1){
all.pools=tmp
