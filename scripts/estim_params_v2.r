...
if (save == TRUE) {
output_file_name = "abcrf_EstimParam_raa_admixture_into_ancestral_pop_prior_000_020_sim30000.txt"
...
### RF parameter
selected.model=1
N.train <- 2000 # Je vais entrainer des forêts sur N.train données
Ntest <- 0  # Tests datasets to compute MNAE, MSE, etc sera fait sur Ntest data of the reference.table not used for training
...
if (param_original==TRUE) {
  ...
  list_original_parameters <- c("raa")
}
...
if (Graphical_density_representation==TRUE) {
  ...
  densityPlot(object = rf.abcrf.NOISE, obs = obs.poi.NOISE, training = train.NOISE, 
              paral = TRUE, bw = 0.05, ylim = c(0, 6), xlim = c(0, 0.2), 
