...
vector.n.train <- list(59000)
vector.n.tree <- list(1000)
N.load.reftable = 60000
...
cat("Closed", length(unused_conns), "unused connections\n")
    ...
    if (CORRELATIONS_COMPILATION_GRAPH==TRUE){
      ...
      # Convertir pred.drf.cor en une matrice carrée de dimension correcte
      n <- length(param.list)
      correlation_matrix <- matrix(correlation_values, nrow = n, ncol = n, byrow = TRUE)
      
      # Vérifier la dimension de la matrice
      print(dim(correlation_matrix))
      
      # Extraire uniquement la moitié inférieure (sans la diagonale)
      correlation_values <- correlation_matrix[lower.tri(correlation_matrix)]
      
      # Afficher le nombre de paires de paramètres conservées
      num_pairs <- length(correlation_values)
      print(paste("Nombre de paires de paramètres conservées :", num_pairs))
      ...
      # Sauvegarder un premier graphique dans un fichier PNG (sans label pour les points)
      png(filename = paste0(output_file_name, "_correlation.graph_raw.png"), width = 800, height = 600)
      ...
      # Sauvegarder un deuxieme graphique dans un fichier PNG (avec toutes les labels)
      # Identifier les paires de paramètres
      pair_indices <- which(lower.tri(correlation_matrix), arr.ind = TRUE)
      pair_names <- apply(pair_indices, 1, function(idx) paste(param.list[idx[1]], param.list[idx[2]], sep = " - "))
      # Afficher le nombre de paires de paramètres conservées
      num_pairs <- length(correlation_values)
      print(paste("Nombre de paires de paramètres conservées :", num_pairs))
      # Supprimer les valeurs proches de -1.0000
      tolerance <- .Machine$double.eps^0.5  # Tolérance pour la précision numérique
      valid_indices <- abs(correlation_values - (-1)) > tolerance
      correlation_values <- correlation_values[valid_indices]
      pair_names <- pair_names[valid_indices]
      png(filename = paste0(output_file_name, "_correlation.graph_with_labels.png"), width = 800, height = 600)
      ...
      plot(correlation_values, 
           ...
           ylim = c(-1, 1),
           cex.axis = 0.6,  # Réduire la taille des caractères des axes
           las = 1)  # Orientation des étiquettes des axes à l'horizontal
      # Ajouter les labels des paires de paramètres
      text(x = 1:length(correlation_values), y = correlation_values, labels = pair_names, pos = 4, cex = 0.6, col = "darkred")
      # Ajouter une échelle des y avec des pas de 0.1
      axis(2, at = seq(-1, 1, by = 0.1), cex.axis = 0.6, las = 1)  # Taille réduite pour l'axe Y
      # Ajouter une ligne horizontale en pointillé à y = 0
      abline(h = 0, lty = 2, col = "red")  # lty = 2 pour pointillé, col = "red" pour visibilité
      ...
      # Sauvegarder un deuxieme graphique dans un fichier PNG (avec ssi les labels pour certains parametres)
      # Identifier les paires de paramètres
      pair_indices <- which(lower.tri(correlation_matrix), arr.ind = TRUE)
      pair_names <- apply(pair_indices, 1, function(idx) paste(param.list[idx[1]], param.list[idx[2]], sep = " - "))
      # Filtrer les paires contenant "raa" ou "ta" ##### Selection des labels
      pair_filter <- grepl("raa|ta", pair_names)
      filtered_correlation_values <- correlation_values[pair_filter]
      filtered_pair_names <- pair_names[pair_filter]
      # Afficher le nombre de paires de paramètres conservées
      num_pairs <- length(correlation_values)
      print(paste("Nombre de paires de paramètres conservées :", num_pairs))
      # Supprimer les valeurs proches de -1.0000
      ...
      png(filename = paste0(output_file_name, "_correlation.graph_labels_for_raa_and_ta_only.png"), width = 800, height = 600)
      # Créer le graphique des correlations
      plot(correlation_values, 
           type = "p", 
           main = "Correlation coefficient for all pairs of parameters", 
           ...
           cex.axis = 0.6,  # Réduire la taille des caractères des axes
           las = 1)  # Orientation des étiquettes des axes à l'horizontale
      # Ajouter les labels des paires filtrées
      text(x = which(pair_filter), y = filtered_correlation_values, labels = filtered_pair_names, pos = 4, cex = 0.6, col = "darkred")
      # Ajouter une échelle des y avec des pas de 0.1
      axis(2, at = seq(-1, 1, by = 0.1), cex.axis = 0.6, las = 1)  # Taille réduite pour l'axe Y
      # Ajouter une ligne horizontale en pointillé à y = 0
      abline(h = 0, lty = 2, col = "red")  # lty = 2 pour pointillé, col = "red" pour visibilité
...
ggplot(data_sorted, aes(x = reorder(PopulationShort, Median))) +
  ...
  POST_TREATMENTS_HPD_MONO_plus_BI_PLOT_and_more =TRUE
...
################## FIN HPD90 mono param #################################
    ...
    ggplot() +
      ...
      labs(
        title = "DRF - HPD Bivariate Plot for ta and raa (introgression of East-Europe from Asia t=2000 sim=50000)" ,
        x = "ta",
        y = "raa",
        color = "HPD Level"
      ) +
