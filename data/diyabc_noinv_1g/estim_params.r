...
if (save == TRUE) {
output_file_name = "abcrf_astra_EstimParam_ta_raa_t1000_sim20000.txt"
...
### RF parameter
selected.model=1
N.train <- 11000 # Je vais entrainer des forêts sur N.train données
...
PLS=FALSE
Graphical_density_representation=TRUE
...
if (param_original==TRUE) {
  ...
  # Liste addoc with name given by the user
  #list_original_parameters = c("raan1","raan2", "raawat", "raasd", "raaas1", "raaas2")
  list_original_parameters <- c("ta", "raa")
}
...
col_index <- 1
  ...
  # Afficher les dimensions et les premières lignes des fichiers clefs pour vérification
  ...
  # head(reference.table$params, n=3)
  # colnames(reference.table$params)
  # dim(reference.table$stats)
  # head(reference.table$stats, n=3)
  # colnames(reference.table$stats)
}
...
# Pour visualiser l'effet du nbre de trees sur erreur d'estimation de la moyenne
plot.name1 = paste0(output_file_name,"_error_vs_ntrees_",parameter_of_interest,".png")
png(file = plot.name1 , width = 800, height = 600)
...
# Ouvrir un fichier PNG pour l'enregistrement
plot.name2 = paste0(output_file_name, "_VarImp.png_",parameter_of_interest,".png")
png(file = plot.name2 , width = 800, height = 600)
...
statobs_predClassic.NOISE <- predict(rf.abcrf.NOISE, obs=obs.poi.NOISE, quantiles = c(0.05,0.95), paral = TRUE,    # quantiles = c(0.05,0.95) and thus 90% CI coverage
...
if (Graphical_density_representation==TRUE) {                         ####### Graphical representation of parameter posterior distributions
  ...
  if (parameter_of_interest=="ta") {
    ...
    densityPlot(object = rf.abcrf.NOISE, obs = obs.poi.NOISE, training = train.NOISE, 
                paral = TRUE, bw = 1000, ylim = c(0, 0.001), xlim = c(100, 10000), 
                lty = 3, main = "Posterior distribution (noPLS)", 
                xlab = "ta", ylab = "Density")
    # Fermer l'appareil de sortie graphique
    dev.off()
  }
  
  if (parameter_of_interest=="raa") {
  output_file_png <- gsub(".txt", ".png", output_file_name)
  output_file_png = paste0(parameter_of_interest,"_", output_file_png)
  ...
  # Générer le density plot
  densityPlot(object = rf.abcrf.NOISE, obs = obs.poi.NOISE, training = train.NOISE, 
              paral = TRUE, bw = 0.05, ylim = c(0, 6), xlim = c(0.05, 0.5), 
              lty = 3, main = "Posterior distribution (noPLS)", 
              xlab = "raa", ylab = "Density")
  # Fermer l'appareil de sortie graphique
  dev.off()
  }
